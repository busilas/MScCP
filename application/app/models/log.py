...
class Log(db.Model):
    ...
    def get_performance_stats():
        ...
        # Guardian statistics
        total_queries = db.session.query(func.count(Log.id)).scalar() or 0
        guardian_flagged = db.session.query(func.count(Log.id)).filter_by(guardian_flag=True).scalar() or 0
