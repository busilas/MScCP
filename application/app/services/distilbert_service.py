...
class DistilBERTService:
    ...
    def _load_local_model(self):
        ...
        try:
            ...
            fine_tuned_path = self._find_latest_model()
            ...
            if fine_tuned_path and os.path.exists(fine_tuned_path):
                ...
                # Load custom intent labels if available
                label_map_path = os.path.join(fine_tuned_path, 'label_map.json')
                if os.path.exists(label_map_path):
                    with open(label_map_path, 'r') as f:
                        label_map = json.load(f)
                        # Reverse mapping: idx -> label
                        self.intents = [label for label, idx in sorted(label_map.items(), key=lambda x: x[1])]
                        print(f"✓ Loaded {len(self.intents)} custom intents")

                print(f"✓ Fine-tuned DistilBERT model loaded from {fine_tuned_path}")
            else:
                ...
                print(f"✓ Base DistilBERT model loaded: {self.model_name} (not trained)")
    ...
    def _find_latest_model(self) -> Optional[str]:
        ...
        # Look for distilbert-* directories
        distilbert_models = sorted(
            [d for d in models_dir.glob("distilbert-*") if d.is_dir()],
            key=lambda x: x.stat().st_mtime,
            reverse=True
        )

        # Return the most recent one that has required files
        for model_path in distilbert_models:
    ...
    def reload_model(self):
        ...
        print("Reloading DistilBERT model...")
        if self.use_local:
            self._load_local_model()
