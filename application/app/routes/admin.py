...
def diagnostic():
    ...
    use_local_llama = os.environ.get('USE_LOCAL_LLAMA', 'False')
    llama_model = os.environ.get('LLAMA_MODEL_NAME', '')

    diagnostic_info = {
        'api_key_present': bool(api_key and api_key != 'your-huggingface-api-key'),
        'api_key_prefix': api_key[:10] + '...' if api_key and len(api_key) > 10 else 'NOT_SET',
        'use_local_llama': use_local_llama,
        'llama_model': llama_model,
        'settings_mode': Settings.get_settings().chatbot_mode,
        'settings_use_local_llama': Settings.get_settings().use_local_llama
    }
...
def dashboard():
    ...
    total_queries = db.session.query(Log).count()
    total_documents = db.session.query(Document).count()
    total_training_files = db.session.query(TrainingFile).count()

    # Recent logs
    recent_logs = db.session.query(Log).order_by(Log.timestamp.desc()).limit(10).all()
...
def documents():
    ...
    """
    all_documents = db.session.query(Document).order_by(Document.uploaded_at.desc()).all()
...
def logs():
    ...
    logs_query = db.session.query(Log).order_by(Log.timestamp.desc())
    ...
    items = logs_query.offset((page - 1) * per_page).limit(per_page).all()
